...
class DioReportRepository implements ReportRepository {
  ...
  @override
  Future<Result<ReportResult>> getReport(
    String reportId,
    ReportQuery query,
  ) async {
    final result = await guard(
      () => _dio.get<Map<String, dynamic>>(
        ...
        queryParameters: {
          ...
          if (query.clinicId != null) 'clinic_id': query.clinicId,
        },
      ),
    );
    ...
    return result.map((response) {
      ...
      if (raw is Map) {
        raw.forEach((key, value) {
          if (value is num) values[key.toString()] = value;
        });
      }
      return ReportResult(
        reportId: reportId,
        generatedAt: DateTime.parse(data['generated_at'].toString()).toUtc(),
        values: values,
      );
    });
  }
